      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan, index) => (
            <div key={index} className={`rounded-2xl p-8 ${plan.popular ? 'bg-primary-600 text-white transform scale-105 shadow-xl relative' : 'bg-white border-2 border-gray-200'}`}>
              {plan.popular && (
                <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-primary-800 px-4 py-1 rounded-full text-sm font-semibold">Most Popular</div>
              )}
              ...
              <Link href={plan.href} className={`block w-full px-6 py-3 rounded-lg font-semibold text-center transition-all ${plan.popular ? 'bg-white text-primary-600 hover:bg-primary-50' : 'border-2 border-primary-600 text-primary-600 hover:bg-primary-50'}`}>
                {plan.cta}
              </Link>
            </div>
          ))}
