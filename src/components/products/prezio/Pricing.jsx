    ...
    <section id="pricing" className="py-24 px-4 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan) => (
            <div key={plan.name} className={`rounded-2xl p-8 ${plan.popular ? 'bg-orange-600 text-white transform scale-105 shadow-xl' : 'bg-white border-2 border-gray-200'}`}>
              ...
              <p className={`mb-6 ${plan.popular ? 'text-orange-100' : 'text-gray-600'}`}>{plan.period}</p>
              <ul className="space-y-3 mb-8">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-2 text-sm">
                    <FiCheckCircle className={`w-5 h-5 flex-shrink-0 mt-0.5 ${plan.popular ? 'text-white' : 'text-orange-600'}`} />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
              <Link href="/contact" className={`block w-full px-6 py-3 rounded-lg font-semibold text-center ${plan.popular ? 'bg-white text-orange-600' : 'border-2 border-orange-600 text-orange-600 hover:bg-orange-50'}`}>
                {plan.cta}
              </Link>
            </div>
          ))}
